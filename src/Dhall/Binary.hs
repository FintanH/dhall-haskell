...
encode_1_1 (Record xTs₀) =
  ...
  where
    xTs₁ = do
        (x₀, _T₀) <- Data.HashMap.Strict.toList xTs₀
        let x₁  = TString x₀
...
encode_1_1 (RecordLit xts₀) =
  ...
  where
    xts₁ = do
        (x₀, t₀) <- Data.HashMap.Strict.toList xts₀
        let x₁ = TString x₀
        let t₁ = encode_1_1 t₀
        return (x₁, t₁)
...
encode_1_1 (UnionLit x t₀ yTs₀) =
  ...
  where
    ...
    yTs₁ = do
        (y₀, _T₀) <- Data.HashMap.Strict.toList yTs₀
        let y₁  = TString y₀
        let _T₁ = encode_1_1 _T₀
        return (y₁, _T₁)
...
decode_1_1 (TList [ TInt 7, TMap xTs₁ ]) = do
    ...
    return (Record (Data.HashMap.Strict.fromList xTs₀))
decode_1_1 (TList [ TInt 8, TMap xts₁ ]) = do
    ...
    return (RecordLit (Data.HashMap.Strict.fromList xts₀))
...
decode_1_1 (TList [ TInt 11, TMap xTs₁ ]) = do
    ...
    return (Union (Data.HashMap.Strict.fromList xTs₀))
decode_1_1 (TList [ TInt 12, TString x, t₁, TMap yTs₁ ]) = do
    ...
    return (UnionLit x t₀ (Data.HashMap.Strict.fromList yTs₀))
